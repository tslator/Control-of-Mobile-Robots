...
%                 fprintf('already in state %s\n', name);
        ...
        %   measured wheel encoder tick counts are stored in prev_ticks.
            ...
            l_dist = (left_ticks - prev_left_ticks)*m_per_tick;
            r_dist = (right_ticks - prev_right_ticks)*m_per_tick;
            c_dist = (l_dist + r_dist)/2;
            
            x_dt = c_dist * cos(theta);
            y_dt = c_dist * sin(theta);
            theta_dt = (r_dist - l_dist)/L;
